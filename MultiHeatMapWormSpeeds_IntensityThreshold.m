...
%% remove data by intensity threshold
    ...
    while colnum <= totalcol
        ...
        for ii = 1:numel(FrameList)
            ...
            currentFrameLogInd = TrajData.frame_number== FrameList(ii);
            nextFrameLogInd = TrajData.frame_number== FrameList(ii) + 1;
            ...
            if nnz(currentFrameLogInd)>nWorms
                ...
                nextFrameWormLogInd = nextFrameLogInd&ismember(TrajData.worm_index_joined,wormInds); %returns logical indices for next frame that have the worm
                overlapLogInd = ismember(wormInds,TrajData.worm_index_joined(nextFrameLogInd)); %returns logical indices for worms that appear in both frames
